...
class GlobalSearchPlugin extends StudIPPlugin implements SystemPlugin {
    ...
    public function show_user_avatar($eventname, $search_item) {
        if ($search_item->type === "user") {
            ...
            if ($GLOBALS['perm']->have_perm("root")) {
                ...
                $search_item->tools[] = '<a href="'.URLHelper::getLink("about.php", array('username' => $uname, 'cmd' => "add_user", 'add_uname' => $uname)).'" title="'._("Als Buddy hinzuf�gen").'">'.Assets::img("icons/16/blue/add/person.png", array('class' => "text-bottom"))."</a>";
            }
        }
    }
    ...
    public function show_seminar_avatar($eventname, $search_item) {
        if ($search_item->type === "seminar") {
            ...
            if ($GLOBALS['perm']->have_perm("admin")) {
                ...
                $search_item->tools[] = '<a href="'.URLHelper::getLink("raumzeit.php", array('cid' => $search_item->entry_id)).'" title="'._("Zeiten/R�ume").'">'.Assets::img("icons/16/blue/date.png", array('class' => "text-bottom"))."</a>";
    ...
    public function search_action() {
        ...
        if (count($_SESSION['search_parameter'])) {
            ...
            if ($_SESSION['search_parameter']['study_area']) {
                //Auf Studienbereich einschr�nken:
                $study_areas = TreeAbstract::getInstance('StudipSemTree', false);
    ...
    public function indexing_action() {
        ...
        if ($GLOBALS['IS_CLI']) {
            echo "Index ersellt mit ".$count." Eintr�gen.";
        } else {
